...
`user-service`、`product-service`、`order-service`、`inventory-service` 通过 MyBatis-Plus `BaseMapper` 访问数据库，并使用 `@MapperScan` 统一扫描 Mapper。简单 CRUD 优先使用 MyBatis-Plus 内置方法，复杂业务仍放在 ServiceImpl 中维护事务边界。
...
成功接口直接返回 `Result<T>`；需要明确 HTTP 状态码的接口使用 `ResponseEntity<Result<T>>`，例如创建资源返回 HTTP 201 且 body 中 `code=201`。业务异常统一使用 `BusinessException` 表达，由 `GlobalExceptionHandler` 转换为匹配 HTTP status 的 `Result.error(...)`。
...
`product-service` 通过 MyBatis-Plus 访问 MySQL 保存商品数据，使用 Redis 做商品详情缓存：
